...
function search(query){return idx.search(query);};(function(){let form=document.querySelector('.new-comment');if(form){form.addEventListener('submit',function(){form.classList.add('loading');form.querySelector('button[type="submit"]:enabled').classList.add('hidden');form.querySelector('button[type="submit"]:disabled').classList.remove('hidden');let api='secretman3.herokuapp.com';let gitProvider='github';let username='VincentTam';let repo='hugo-future-imperfect-slim';let branch='addGHCI';let url=['https:/',api,'v3/entry',gitProvider,username,repo,branch,'comments'].join('/');let xhr=new XMLHttpRequest();xhr.open('POST',url);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.setRequestHeader('X-Requested-With','XMLHttpRequest');xhr.onreadystatechange=function(){if(xhr.readyState===XMLHttpRequest.DONE){let status=xhr.status;if(status>=200&&status<400){formSubmitted();}else{formError();}}};xhr.send(urlencodeFormData(new FormData(form)));return false;});function urlencodeFormData(fd){let s='';function encode(s){return encodeURIComponent(s).replace(/%20/g,'+');}
for(var pair of fd.entries()){if(typeof pair[1]=='string'){s+=(s?'&':'')+encode(pair[0])+'='+encode(pair[1]);}}
return s;}
function formSubmitted(){showAlert('success');setTimeout(function(){clearForm()},3000);form.classList.remove('loading');}
